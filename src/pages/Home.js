...
const Home = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('http://localhost:5296/api/product?IsDeleted=false');
        const items = response.data?.items || [];
        
        // If we have more than 4 items, randomly select 4
        if (items.length > 4) {
          ...
          const selectedProducts = Array.from(selectedIndices).map(index => items[index]);
          setFeatureProducts(selectedProducts);
        } else {
          // If we have 4 or fewer items, use all of them
          setFeatureProducts(items);
        }
      } catch (error) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12 dark:text-white">Feature Products</h2>
          {isLoading ? (
