...
const BrandModal = ({ open, onClose, onSave, editData = null }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editData) {
                await axios.put(`/api/brand/${editData.id}`, submitData, {
                    headers: {
    ...
    const handleClose = () => {
        if (formData.name || formData.description || formData.origin) {
    ...
    const handleConfirmClose = () => {
        setFormData({ name: '', description: '', origin: '' });
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-2xl font-bold">
                                {editData ? 'Edit Brand' : 'Create New Brand'}
                            </h2>
                            <button
                                onClick={handleClose}
