  ...
  const fetchPosts = async () => {
    try {
      ...
      const draftResponse = await Axios.get(
        `/api/post?IsDeleted=false&PageNumber=${draftPage}&PageSize=10&Status=false${
          draftSearch ? `&Title=${draftSearch}` : ""
        }`
      );
      setDraftPosts({
      ...
      const publishedResponse = await Axios.get(
        `/api/post?IsDeleted=false&PageNumber=${publishedPage}&PageSize=10&Status=true${
          publishedSearch ? `&Title=${publishedSearch}` : ""
        }`
      );
      setPublishedPosts({
      ...
      <table className="min-w-full bg-white border border-gray-300 table-fixed">
        <thead>
          <tr>
            ...
            <th className="py-2 px-4 border-b text-left w-40">
              Published Date
            </th>
        ...
        <tbody>
          {isLoading ? (
            ...
            posts.items.map((post) => (
              <tr key={post.id}>
                <td className="py-2 px-4 border-b truncate w-24">{post.id}</td>
                ...
                <td className="py-2 px-4 border-b truncate w-32">
                  {post.author}
                </td>
