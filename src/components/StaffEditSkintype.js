...
const StaffEditSkintype = ({ open, onClose, onSave, skintype }) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (error.response?.data?.message) {
                ...
                toast.error('Failed to update skintype. Please try again.');
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const handleClose = () => {
        if (formData.name !== skintype.name || formData.description !== skintype.description) {
    ...
    return (
        <>
            <Modal
                ...
                style={{ zIndex: 1000 }}
                aria-labelledby="edit-skintype-modal"
            >
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-2xl bg-white rounded-lg shadow-xl p-6 max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold">Edit Skintype</h2>
                        <button
                            onClick={handleClose}
...
export default StaffEditSkintype; 